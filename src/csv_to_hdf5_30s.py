...
# 📁 디렉토리 설정
# ==========================================
INPUT_DIR = r'F:\coding자료\coding\digital_hearth_care\test_set\IHD_30s'      # CSV 파일 디렉토리
OUTPUT_FILE = r'F:\coding자료\coding\digital_hearth_care\test_set\IHD_30s.h5'  # 출력 HDF5 파일
...
DURATION = 30               # seconds
SEQUENCE_LENGTH = SAMPLING_RATE * DURATION  # 7680 samples
# ==========================================


def csv_to_hdf5(input_dir, output_file, compression='gzip', compression_level=4):
    """
    CSV 파일들을 HDF5 포맷으로 변환 (30초 세그먼트)
    ...
    print(f"⏱️  세그먼트 길이: {DURATION}초")
    print(f"📊 샘플링 레이트: {SAMPLING_RATE} Hz")
    print(f"📐 예상 시퀀스 길이: {SEQUENCE_LENGTH} samples")
    print("="*70)
    ...
    if n_samples != SEQUENCE_LENGTH:
        print(f"\n⚠️  경고: 실제 샘플 수({n_samples})가 예상({SEQUENCE_LENGTH})과 다릅니다!")
        print(f"   실제 데이터 길이로 진행합니다: {n_samples} samples")
        actual_duration = n_samples / SAMPLING_RATE
        print(f"   실제 지속 시간: {actual_duration:.2f}초")
    else:
        print(f"\n✓ 시퀀스 길이 검증 완료: {n_samples} samples (30초)")
    
    print(f"\n📊 데이터 정보:")
    print(f"  - 파일당 샘플 수: {n_samples:,}")
    print(f"  - 컬럼: {list(first_df.columns)}")
    print(f"  - 전체 데이터 포인트: {n_files * n_samples:,}개")
    print(f"  - 전체 데이터 시간: {(n_files * n_samples / SAMPLING_RATE / 60):.1f}분")
    print()
    ...
    for idx, csv_file in enumerate(tqdm(csv_files, desc="Loading")):
        ...
        # 길이 검증
        if len(df) != n_samples:
            print(f"\n⚠️  경고: {csv_file.name}의 샘플 수가 다릅니다 ({len(df)} vs {n_samples})")
            # 패딩 또는 자르기
            if len(df) < n_samples:
                # 부족한 경우 제로 패딩
                ecg_data[idx, :len(df)] = df['ecg'].values
                ppg_data[idx, :len(df)] = df['ppg'].values
                time_data[idx, :len(df)] = df['time'].values
            else:
                # 초과한 경우 자르기
                ecg_data[idx] = df['ecg'].values[:n_samples]
                ppg_data[idx] = df['ppg'].values[:n_samples]
                time_data[idx] = df['time'].values[:n_samples]
        else:
    ...
    with h5py.File(output_file, 'w') as hf:
        ...
        hf.attrs['description'] = 'Preprocessed 30-second ECG and PPG signals for cardiac anomaly detection'
        hf.attrs['total_duration_minutes'] = (n_files * DURATION) / 60
        print("  ✓ 메타데이터 저장 완료")
